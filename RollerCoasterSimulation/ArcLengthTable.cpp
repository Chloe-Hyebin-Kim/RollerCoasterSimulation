...
ArcSample ArcLengthTable::FrameAtArcLength(const CatmullRomSpline& spline, float s) const
{
    ...
    const ArcSample& bSample = *it;
    const ArcSample& aSample = *(it - 1);
...
/// <returns></returns>
float ArcLengthTable::CalParamFromLength(float f32D) const
{
    ...
    const ArcSample& lowerSample = m_arrSamples[i32UpperIndex - 1];
    const ArcSample& upperSample = m_arrSamples[i32UpperIndex];
